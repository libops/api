...
SELECT id, organization_id, account_id, `role`, created_at, updated_at, created_by, updated_by
FROM organization_members WHERE organization_id = ? AND account_id = ?;

-- name: CreateOrganizationMember :exec
INSERT INTO organization_members (
  organization_id, account_id, `role`, created_at, updated_at, created_by, updated_by
) VALUES (?, ?, ?, NOW(), NOW(), ?, ?);
...
SELECT cm.id, cm.organization_id, cm.account_id, cm.`role`, cm.status, cm.created_at, cm.updated_at,
...
SELECT id, project_id, account_id, `role`, created_at, updated_at, created_by, updated_by
FROM project_members WHERE project_id = ? AND account_id = ?;

-- name: CreateProjectMember :exec
INSERT INTO project_members (
  project_id, account_id, `role`, created_at, updated_at, created_by, updated_by
) VALUES (?, ?, ?, NOW(), NOW(), ?, ?);
...
SELECT pm.id, pm.project_id, pm.account_id, pm.`role`, pm.status, pm.created_at, pm.updated_at,
...
SELECT id, site_id, account_id, `role`, created_at, updated_at, created_by, updated_by
FROM site_members WHERE site_id = ? AND account_id = ?;

-- name: CreateSiteMember :exec
INSERT INTO site_members (
  site_id, account_id, `role`, created_at, updated_at, created_by, updated_by
) VALUES (?, ?, ?, NOW(), NOW(), ?, ?);
...
SELECT sm.id, sm.site_id, sm.account_id, sm.`role`, sm.status, sm.created_at, sm.updated_at,
       BIN_TO_UUID(a.public_id) AS account_public_id, a.email, a.`name`, a.github_username
